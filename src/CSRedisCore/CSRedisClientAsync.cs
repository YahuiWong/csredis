...
namespace CSRedis
{
    public partial class CSRedisClient
    {
        ...
        Task<T> NodesNotSupportAsync<T>(string key, Func<Object<RedisClient>, string, Task<T>> callback)
        {
            ...
            return ExecuteScalarAsync<T>(key, "NodesNotSupportAsync<T>", callback);
        }
        ...
        async public Task<bool> PfAddAsync<T>(string key, params T[] elements)
        {
            ...
            return await ExecuteScalarAsync(key, "PfAddAsync<T>", (c, k) => c.Value.PfAddAsync(k, args));
        }
        /// <summary>
        ...
        /// <param name="count">数量</param>
        /// <returns></returns>
        async public Task<(T member, decimal score)[]> ZPopMaxAsync<T>(string key, long count) => this.DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key, "ZPopMaxAsync<T>", (c, k) => c.Value.ZPopMaxBytesAsync(k, count)));
        /// <summary>
        ...
        /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
        /// <returns></returns>
        async public Task<T[]> ZRangeAsync<T>(string key, long start, long stop) => this.DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, "ZRangeAsync<T>", (c, k) => c.Value.ZRangeBytesAsync(k, start, stop, false)));
        /// <summary>
        ...
        /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
        /// <returns></returns>
        async public Task<(T member, decimal score)[]> ZRangeWithScoresAsync<T>(string key, long start, long stop) => this.DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key, "ZRangeWithScoresAsync<T>", (c, k) => c.Value.ZRangeBytesWithScoresAsync(k, start, stop)));
        ...
        async public Task<T[]> ZRangeByScoreAsync<T>(string key, decimal min, decimal max, long? count = null, long offset = 0) =>
            this.DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, "ZRangeByScoreAsync<T>", (c, k) => c.Value.ZRangeBytesByScoreAsync(k, min == decimal.MinValue ? "-inf" : min.ToString(), max == decimal.MaxValue ? "+inf" : max.ToString(), false, offset, count)));
        /// <summary>
        /// 通过分数返回有序集合指定区间内的成员
        /// </summary>
        /// <param name="key">不含prefix前辍</param>
        ...
        async public Task<T[]> ZRangeByScoreAsync<T>(string key, string min, string max, long? count = null, long offset = 0) =>
            this.DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, "ZRangeByScoreAsync<T>", (c, k) => c.Value.ZRangeBytesByScoreAsync(k, min, max, false, offset, count)));
        ...
        async public Task<(T member, decimal score)[]> ZRangeByScoreWithScoresAsync<T>(string key, decimal min, decimal max, long? count = null, long offset = 0) =>
            this.DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key, "ZRangeByScoreWithScoresAsync<T>", (c, k) => c.Value.ZRangeBytesByScoreWithScoresAsync(k, min == decimal.MinValue ? "-inf" : min.ToString(), max == decimal.MaxValue ? "+inf" : max.ToString(), offset, count)));
        /// <summary>
        /// 通过分数返回有序集合指定区间内的成员和分数
        /// </summary>
        /// <param name="key">不含prefix前辍</param>
        ...
        async public Task<(T member, decimal score)[]> ZRangeByScoreWithScoresAsync<T>(string key, string min, string max, long? count = null, long offset = 0) =>
            this.DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key, "ZRangeByScoreWithScoresAsync<T>", (c, k) => c.Value.ZRangeBytesByScoreWithScoresAsync(k, min, max, offset, count)));
        ...
        async public Task<long> ZRemAsync<T>(string key, params T[] member)
        {
            ...
            return await ExecuteScalarAsync(key, "ZRemAsync<T>", (c, k) => c.Value.ZRemAsync(k, args));
        }
        /// <summary>
        /// 移除有序集合中给定的排名区间的所有成员
        /// </summary>
        ...
        /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
        /// <returns></returns>
        async public Task<T[]> ZRevRangeAsync<T>(string key, long start, long stop) => this.DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, "ZRevRangeAsync<T>", (c, k) => c.Value.ZRevRangeBytesAsync(k, start, stop, false)));
        /// <summary>
        ...
        /// <param name="stop">结束位置，0表示第一个元素，-1表示最后一个元素</param>
        /// <returns></returns>
        async public Task<(T member, decimal score)[]> ZRevRangeWithScoresAsync<T>(string key, long start, long stop) => this.DeserializeRedisValueTuple1Internal<T, decimal>(await ExecuteScalarAsync(key, "ZRevRangeWithScoresAsync<T>", (c, k) => c.Value.ZRevRangeBytesWithScoresAsync(k, start, stop)));
        ...
        async public Task<T[]> ZRevRangeByScoreAsync<T>(string key, decimal max, decimal min, long? count = null, long offset = 0) =>
            this.DeserializeRedisValueArrayInternal<T>(await ExecuteScalarAsync(key, "ZRevRangeByScoreAsync<T>", (c, k) => c.Value.ZRevRangeBytesByScoreAsync(k, max == decimal.MaxValue ? "+inf" : max.ToString(), min == decimal.MinValue ? "-inf" : min.ToString(), false, offset, count)));
        /// <summary>
        /// 返回有序集中指定分数区间内的成员，分数从高到低排序
        /// </summary>
        /// <param name="key">不含prefix前辍</param>
        ...
        async public Task<long> RPushAsync<T>(string key, params T[] value)
        {
            if (value == null || value.Any() == false) return 0;
            var args = value.Select(z => this.SerializeRedisValueInternal(z)).ToArray();
            return await ExecuteScalarAsync(key, "RPushAsync<T>", (c, k) => c.Value.RPushAsync(k, args));
        }
        /// <summary>
        /// 为已存在的列表添加值
        /// </summary>
        ...
        /// <param name="keys">不含prefix前辍</param>
        /// <returns></returns>
        public Task<string[]> MGetAsync(params string[] keys) => ExecuteArrayAsync(keys, "IncrByFloatAsync", (c, k) => c.Value.MGetAsync(k));
        /// <summary>
        /// 获取多个指定 key 的值(数组)
        /// </summary>
        /// <typeparam name="T">byte[] 或其他类型</typeparam>
